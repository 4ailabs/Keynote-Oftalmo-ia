    ...
    return (
        <section ref={ref} id="hero" className="min-h-screen h-screen flex flex-col justify-center items-center text-center p-4 relative overflow-hidden bg-black bg-[radial-gradient(ellipse_at_center,_rgba(38,38,38,1)_0%,_rgba(0,0,0,1)_70%)]">
            ...
            <div className={`relative z-10 max-w-4xl mx-auto transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                ...
                <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold tracking-tighter mb-6 bg-gradient-to-r from-blue-500 to-cyan-400 bg-clip-text text-transparent animate-gradient-x">
                ...
                {/* Animated subtitle */}
                <p className={`max-w-3xl mx-auto text-xl md:text-2xl text-zinc-400 mb-12 transform transition-all duration-1000 delay-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'}`}>
                ...
                {/* Animated buttons */}
                <div className={`flex flex-col sm:flex-row justify-center items-center gap-4 transform transition-all duration-1000 delay-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'}`}>
                    <button 
                        onClick={() => scrollToSection('intro')}
                        className="group w-full sm:w-auto px-8 py-3 bg-blue-600 text-white rounded-full font-medium text-lg transition-all duration-300 hover:bg-blue-700 hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-600/30 hover:scale-105 active:scale-95">
                    ...
                    <button 
                        ...
                        className="group w-full sm:w-auto px-8 py-3 bg-transparent border border-zinc-600 text-zinc-100 rounded-full font-medium text-lg transition-all duration-300 hover:bg-zinc-800 hover:border-zinc-500 hover:-translate-y-1 hover:scale-105 active:scale-95 relative overflow-hidden">
            ...
            <div className={`absolute bottom-10 left-1/2 -translate-x-1/2 text-zinc-500 flex flex-col items-center gap-2 transform transition-all duration-1000 delay-1500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'}`}>
                <span className="text-sm animate-pulse">Desliza para explorar</span>
